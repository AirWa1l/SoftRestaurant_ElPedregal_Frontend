...
export function NewOrderPage() {
  ...
  const [orderNumber, setOrderNumber] = useState('039')
  ...
  useEffect(() => {
    ...
    // Fetch order number to show at top
    ...
    async function loadData() {
      ...
      try {
        ...
        if (mounted) {
          ...
          // Initial load from catalog cart (pedregal_cart) if items exist
          const cachedCart = window.localStorage.getItem('pedregal_cart')
          if (cachedCart && loadedProducts.length > 0) {
            try {
              const parsedCart = JSON.parse(cachedCart) as Record<string, number>
        ...
        if (mounted) {
          toast.current?.show({
            severity: 'error',
            summary: 'Error',
            detail: 'No fue posible cargar los datos.',
            life: 4000,
          })
        }
      } finally {
  ...
  function handleAddToCart(product: Product) {
    setCart((prev) => {
      ...
      // Sync with local catalog cart
      const syncCart: Record<string, number> = {}
      Object.entries(nextCart).forEach(([id, item]) => {
        syncCart[id] = item.quantity
      })
      window.localStorage.setItem('pedregal_cart', JSON.stringify(syncCart))
        ...
        {isLoading ? (
          ...
          <div className="grid align-items-stretch" style={{ minHeight: 'calc(100vh - 40px)' }}>
            ...
            <div className="col-12 lg:col-8 flex flex-column gap-3 p-3">
              ...
              {/* Product grid displaying actual image, fallback to emoji + pastel bg */}
              {filteredProducts.length === 0 ? (
                <div className="surface-card border-round-xl border-1 surface-border p-8 text-center flex flex-column align-items-center justify-content-center">
                  ...
                  <p className="text-sm text-500 m-0 mt-1">No hay productos en esta categoría o que coincidan con la búsqueda.</p>
                </div>
              ) : (
                <div className="grid">
                  {filteredProducts.map((prod) => {
                    ...
                    return (
                      <div key={prod.id} className="col-12 sm:col-6 md:col-4">
                        ...
                        >
                          {/* Upper section: actual image or emoji fallback with pastel color */}
                          <div
            ...
            {/* RIGHT COLUMN: Receipt Checkout Card */}
              ...
              >
                <div className="flex flex-column gap-4">
                  ...
                  <div className="flex justify-content-between align-items-center">
                    <h2 className="text-base font-extrabold text-900 m-0">Pedido #{orderNumber}</h2>
                        ...
                        {Object.values(cart).map((item) => {
                          ...
                          return (
                            <div key={item.product.id} className="flex align-items-center justify-content-between gap-2">
                              {/* Left item details with image or fallback emoji */}
                ...
                <div className="flex flex-column gap-3 mt-auto">
                  ...
                  <div className="flex flex-column gap-2 mt-2">
                    <Button
                      label={isSubmitting ? 'Registrando pedido...' : 'Registrar pedido'}
